...
/**
 * Implementation of the BaseListener generated by antlr4.
 */
public class LTLListener extends LTLBaseListener {
    private Formula ltlTree;
    ...
    /**
     * This function takes a node from the parse tree generated by antlr, creates a node of type Formula and
     ...
     * @return The node of the LTL tree; its type is not Formula but one of its subtypes
     */
    private Formula createLTLFormula(ParseTree ctx) {
        if (ctx instanceof LTLParser.FormulafContext) {
            return new F(createLTLFormula(ctx.getChild(1)));
        }
            ...
            return new X(createLTLFormula(ctx.getChild(1)));
        }
            ...
            // The relevant children are at indices 0 and 2. Unlike AND and OR, U is a strictly binary operator.
                ...
                return new U(createLTLFormula(ctx.getChild(0)), createLTLFormula(ctx.getChild(2)));
            }
        }
        else {
    ...
    private class LTLVariablePair {
        Variable negatedVariable;
        Variable nonNegatedVariable;
        String symbol;

        LTLVariablePair(String symbol) {
            this.symbol = symbol;
            this.negatedVariable = new Variable(symbol, true);
            this.nonNegatedVariable = new Variable(symbol);
        }

        public Variable getNegatedVariable() {
            return negatedVariable;
        }

        public Variable getNonNegatedVariable() {
